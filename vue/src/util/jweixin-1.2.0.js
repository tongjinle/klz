!function(a, b) {
  ...
  function c(b, c, d) {
    ...
    a == 'openEnterpriseChat' && (b.errCode = b.err_code), delete b.err_code, delete b.err_desc, delete b.err_detail;let d = b.errMsg

    d || (d = b.err_msg, delete b.err_msg, d = i(a, d), b.errMsg = d), c = c || {}, c._complete && (c._complete(b), delete c._complete), d = b.errMsg || '', D.debug && !c.isInnerInvoke && alert(JSON.stringify(b));let e = d.indexOf(':'),
      f = d.substring(e + 1)

    switch (f) {
      case 'ok':c.success && c.success(b);break;case 'cancel':c.cancel && c.cancel(b);break;default:c.fail && c.fail(b)
  ...
  if (!(v || w || D.debug || A < '6.0.2' || C.systemType < 0)) {
    ...
    C.appId = D.appId, C.initTime = B.initEndTime - B.initStartTime, C.preVerifyTime = B.preVerifyEndTime - B.preVerifyStartTime, I.getNetworkType({
      isInnerInvoke: !0, success(a) {
        C.networkType = a.networkType;let c = `https://open.weixin.qq.com/sdk/report?v=${C.version}&o=${C.isPreVerifyOk}&s=${C.systemType}&c=${C.clientVersion}&a=${C.appId}&n=${C.networkType}&i=${C.initTime}&p=${C.preVerifyTime}&u=${C.url}`
  ...
  var p = {config: 'preVerifyJSAPI', onMenuShareTimeline: 'menu:share:timeline', onMenuShareAppMessage: 'menu:share:appmessage', onMenuShareQQ: 'menu:share:qq', onMenuShareWeibo: 'menu:share:weiboApp', onMenuShareQZone: 'menu:share:QZone', previewImage: 'imagePreview', getLocation: 'geoLocation', openProductSpecificView: 'openProductViewWithPid', addCard: 'batchAddCard', openCard: 'batchViewCard', chooseWXPay: 'getBrandWCPayRequest', openEnterpriseRedPacket: 'getRecevieBizHongBaoRequest', startSearchBeacons: 'startMonitoringBeacons', stopSearchBeacons: 'stopMonitoringBeacons', onSearchBeacons: 'onBeaconsInRange', consumeAndShareCard: 'consumedShareCard', openAddress: 'editAddress'},
    ...
    A = function() {
      let a = t.match(/micromessenger\/(\d+\.\d+\.\d+)/) || t.match(/micromessenger\/(\d+\.\d+)/)
  ...
  n(() => {
    ...
    I = {
      config(a) {
        D = a, k('config', a);let b = D.check === !1 ? !1 : !0
        ...
        n(() => {
          if (b) {
            ...
            F.state = 1;for (let a = E._completes, d = 0, e = a.length;e > d;++d) {
              a[d]()
            }E._completes = []
          }
        }), D.beta && o()
  ...
  let b = function(a) {
    ...
    a.errMsg = 'getNetworkType:ok';let c = a.subtype

    if (delete a.subtype, c) {
      a.networkType = c
    } else {
      let d = b.indexOf(':'),
        e = b.substring(d + 1)
  ...
  for (var b = a.cardList, d = [], e = 0, f = b.length;f > e;++e) {
    ...
    return a._complete = function(a) {
      ...
      if (b) {
        b = JSON.parse(b);for (let c = 0, d = b.length;d > c;++c) {
          let e = b[c]
  ...
  return r.addEventListener('error', a => {
    if (!y) {
      ...
      if (c == 'IMG' || c == 'VIDEO' || c == 'AUDIO' || c == 'SOURCE') {
        let e = d.indexOf('wxlocalresource://') != -1

        if (e) {
          a.preventDefault(), a.stopPropagation();let f = b['wx-id']

          if (f || (f = J++, b['wx-id'] = f), K[f]) {
            return
          }K[f] = !0, wx.ready(() => {
            wx.getLocalImgData({
              localId: d, success(a) {
    ...
    if (!y) {
      ...
      b.src;if (c == 'IMG' || c == 'VIDEO' || c == 'AUDIO' || c == 'SOURCE') {
        let d = b['wx-id']
